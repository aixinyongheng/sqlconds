...
class BasesqlConds {
  ...
  public RelationSign: Record<string, string> = {
  ...
  private _formatCondsql({ conditem, rntable }: any) {
    ...
    if (conditem.operator.toUpperCase() === 'FQ' || conditem.operator.toUpperCase() === 'NFQ') {
      ...
      itemvalue = this.dealdifferentItemvalue({ conditem, rntable });
  ...
  dealdifferentConds(conditem: any, conditemFieldReset: any, itemvalue: any) {
    return ` ${conditemFieldReset} ${this.RelationSign[conditem.operator.toUpperCase()]}  ${itemvalue} `;
  }
  ...
  private _statisCondPackage(conds: any, sqlconds: any) {
    const StaticsRelationSign = (key: string, field?: string, typecast?: string) => {
      const _: any = {
    ...
    for (const item of conds) {
      ...
      if (item.type === 'ZDY' && item.aggSign && item.optSign) {
        ...
        fieldArr.forEach((field: any) => {
          if (item.typecast) {
            fieldList.push(`${StaticsRelationSign(item.aggSign, field, '::' + item.typecast)}`);
          } else {
            fieldList.push(`${StaticsRelationSign(item.aggSign, field, '')}`);
          }
        });
        aggObj = fieldList.join(StaticsRelationSign(item.optSign));
      } else {
        if (item.typecast) {
          aggObj = `${StaticsRelationSign(item.type, item.field, '::' + item.typecast)}`;
        } else {
          aggObj = `${StaticsRelationSign(item.type, item.field, '')}`;
        }
      }
      if (item.dpoint) {
